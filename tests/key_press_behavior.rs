    ...
    for ev in input.read() {
        match ev.state {
            ButtonState::Pressed => {
                ...
                println!(
                    "Frame {}: Saw PRESS event for {:?} (key_code: {:?}, logical_key: {:?})",
            ...
            ButtonState::Released => {
                ...
                println!(
                    "Frame {}: Saw RELEASE event for {:?} (key_code: {:?}, logical_key: {:?})",
                    current_frame, ev.key_code, ev.logical_key, ev.logical_key
                );
            }
        }
...
fn test_keyboard_event_persistence() {
    ...
    println!("\n=== Summary ===");
    println!(
        "Press events seen at frames: {:?}",
        tracker.press_events_seen
    );
    println!(
        "Release events seen at frames: {:?}",
        tracker.release_events_seen
    );
    println!("Total frames: {}", tracker.current_frame);

    // Assertions
    assert!(
        !tracker.press_events_seen.is_empty(),
        "Should have seen at least one press event"
    );
    assert!(
        !tracker.release_events_seen.is_empty(),
        "Should have seen at least one release event"
    );

    // Check that events are only seen once (MessageReader should consume them)
    assert_eq!(
        tracker.press_events_seen.len(),
        1,
        "Press event should only be seen once (consumed by MessageReader)"
    );
    assert_eq!(
        tracker.release_events_seen.len(),
        1,
